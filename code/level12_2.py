...
with open(filename) as file:
    ...
    for record in records:
        j += 1
        record = record.split(" ")
        doubled_springs = "?".join([record[0] for _ in range(5)])
        doubled_conditions = [el for _ in range(5) for el in record[1] if el != ","]
        springs = [
            rec.replace("#", "1") for rec in doubled_springs.split(".") if rec != ""
        ]
        conditions = list(map(int, doubled_conditions))
